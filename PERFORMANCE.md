...
针对 EPUB 图书实现的重大性能优化：
- **服务器代理模式**: 新增 `/api/books/[id]/proxy/` 接口，支持按需提取 EPUB 压缩包内的单个文件。
- **减少带宽占用**: 浏览器不再一次性下载整本图书（通常几MB到几十MB），而是仅在阅读到对应章节时请求几KB的资源。
- **提升首屏加载速度**: 大文件的加载时间从原来的数秒（等待完整下载）降至毫秒级。
- **离线支持兼容**: 保留了 IndexedDB 本地缓存机制，已下载的图书仍可离线访问。
